...
tasklist 命令用于显示计算机上正在运行的进程列表

可以先使用netstat -ano查看端口连接信息，然后使用tasklist | findstr PID 进行排查

```bash
tasklist /svc #显示正在运行的进程及其关联的服务

tasklist /m #列出当前运行的进程以及每个进程加载的模块（DLL）
```

wmic process 命令可用来获取进程相关的信息，常用命令如下：

```bash
wmic process get name,ExecutablePath,processid,parentprocessid /format: csv

# 以csv格式来显示进程的名称、可执行文件路径、进程ID、父进程ID信息

wmic process where processid=[PID] get parentprocessid

# 以PID的值作为条件来获取其父进程的PID
```

## Windows用户排查

在命令行中输入【`wmic useraccount get name,sid`】命令，可查看到系统当中的所有用户的用户名以及SID信息。

在运行窗口输入【regedit】打开【注册表编辑器】窗口，选择【HKEY_LOCAL_MACHINE】—>【SAM】选项，为该项添加【使用可从此对象继承的权限项目替换所有子对象的权限项目】权限，使当前用户拥有SAM的读取权限，添加权限完成后按【F5】键，刷新后即可访问子项并查看用户信息，然后继续选择【SAM】—>【Domains】—>【Account】—>【Users】—>【Names】查看是否有异常的账户。

由于放置在特定的启动文件夹中的程序在用户登录时系统会自动执行，所以这些目录经常被攻击者用来自启动后门，系统当中常见的系统启动文件夹如下。
...
注册表是Microsoft Windows中的一个重要数据库、用于存储系统和应用程序的系统信息，下方为Windows注册表当中的五大根键
...
HKU：该根键包括计算机的所有用户使用的配置数据，这些数据只有在用户登录系统时才能访问

HKCC：该根键包括当前硬件的配置信息，其中的信息是从HKEY_LOCAL_MACHINE中映射出来的
```


Windows系统上有许多注册表项可以用来设置在系统启动或用户登录时运行指定的程序，很多攻击者会对此类注册表进行滥用，以建立持久化后门，下列是一些常见的注册表启动项。

1）Run键是最常见的自启动注册表项，该键位置是
...
```bash
[HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Windows]下的load键值的程序也可以自启动。
```
...
下面的Notify、Userinit、Shell键值也会有自启动的程序，而且其键值可以用逗号分隔，从而实现登录的时候启动多个程序。 

## Windows计划任务排查

打开【计算机管理】—>【系统工具】—>【任务计划程序】—>【任务计划程序库】选项或者执行【taskschd.msc】命令，可以查看任务计划的名称、状态、触发器等详细信息。

在命令行中输入【schtasks】命令，可获取任务计划的信息，该命令是一个功能强大的超级命令行计划工具，获取任务计划时要求必须是本地Administrators组的成员。

使用 【schtasks /query /v /fo LIST】 命令可获取计划任务的详细信息

## Windows服务排查

排查时打开【运行】对话框，输入【services.msc】命令，便可打开服务窗口，查看所有的服务项，包括服务的名称、描述、状态等

## IFEO注入（映像劫持）排查

IFEO（Image File Execution Options）是Windows系统的一个注册表项，路径为 `\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options`。

当用户启动计算机的程序后，系统会在注册表的IFEO中查询所有的程序子键，如果存在与该程序名称相同的子健，就读取对应子键的 Dubugger 键值。如果该键值未被设置，就默认不做处理，否则直接用该键值所指定的程序路径来代替原始的程序。通过编辑 Dubugger 的值，攻击者可以通过修改注册表的方式创建粘滞键后门，而不需获取 TrustedInstaller 权限。

## Windows系统日志简介

Windows日志记录着Windows系统中硬件、软件和系统问题的信息，同时还可以监视系统中发生的事件，掌握计算机在特定时间的状态，以及了解用户的各种操作行为，为应急响应提供很多关键信息。

### Windows系统日志分类

#### 1）系统日志

系统日志记录系统中各种驱动程序在运行中出现的重大问题、操作系统的多种组件在运行中出现的重大问题及应用软件在运行中出现的重大问题等。
...
安全性日志主要记录了各种与安全相关的事件。构成该日志的内容主要包括：各种登录与退出系统的成功或不成功的信息、特权使用、账号管理、策略变更等各类日志内容。
...
上述三类日志的存储位置如下
	...
	在Windows 2000 专业版/Windows XP/Windows Server 2003（注意日志文件的后缀名是evt）系统中：
		...
		应用程序日志的位置为 C:\WINDOWS\system32\config\AppEvent.Evt
...
Windows系统中的每个事件都有其相应的事件ID，下图是应急响应中常用的事件ID

- 旧版本指Windows 2000 专业版/Windows XP/Windows Server 2003，
- 新版本指Windows Vista/Windows 7/Windows 8/Windows 10/Windows Server 2008等。

![image-20241029174701584](https://image.201068.xyz/assets/38.后渗透/image-20241029174701584.png)

Windows系统将登录类型显示为数字，下图是数字及其对应说明。

![image-20241029174713089](https://image.201068.xyz/assets/38.后渗透/image-20241029174713089.png)

登录相关日志事件ID对应的描述。

![image-20241029174719455](https://image.201068.xyz/assets/38.后渗透/image-20241029174719455.png)

日志清除相关时间ID

![image-20241029174725388](https://image.201068.xyz/assets/38.后渗透/image-20241029174725388.png)

## 常用工具

### 火绒剑

火绒剑是一款用于分析、处理程序的安全工具。

它提供了“程序行为监控”、“进程管理”、“启动项管理”、“内核程序管理”、“钩子扫描”、“服务管理”、“驱动扫描”、“网络监控”、“文件管理”、“注册表管理”十大功能。

### Autoruns

AutoRuns是一款出色的启动项目管理工具，它能用于显示在 Windows启动或登录时自动运行的程序，并且允许用户有选择地禁用或删除它们，例如那些在“启动”文件夹和注册表相关键中的程序。此外，Autoruns还可以修改包括：Windows 资源管理器的 Shell 扩展（如右键弹出菜单）、IE浏览器插件（如工具栏扩展）、系统服务和设备驱动程序、计划任务等多种不同的自启动程序。

### PCHunter

PCHunter：PCHunter是一个功能强大的Windows系统信息查看软件，同时也是手工杀毒软件，它不但可以查看各类系统信息，还可以帮助我们查出计算机中潜伏的木马

### Process Explorer

Process Explorer：使用者能通过该工具了解看不到的在后台执行的处理程序，能显示已经载入哪些模块，分别是正在被哪些程序使用着，还可显示这些程序所调用的 DLL进程，以及他们所打开的句柄。

### D盾

专为IIS设计的一个主动防御的保护软件，具有主动后门拦截，SESSION保护，防WEB嗅探，防CC，防篡改，注入防御，防XSS，防提权，上传防御，未知0day防御，异形脚本防御等等。
...
某天客户反馈：服务器疑似被入侵，风扇噪声很大，CPU占用过高。

**受害服务器：** Windows2008 系统
...
根据客户反馈：“**风扇噪声很大**”，一般只有消耗CPU很多的情况下，服务器高温，风扇才会一直转，说明服务器可能**感染挖矿病毒**了

### 2.1 排查服务器是否感染挖矿病毒

#### java程序

登录进服务器之后，看到桌面下面有一个`java`程序在运行

![image-20220912161347245](https://image.201068.xyz/assets/38.后渗透/image-20220912161347245.png)

 点开java图标之后，发现该程序一直在访问域名：`mine.c3pool.com:13333`

![image-20220912161524169](https://image.201068.xyz/assets/38.后渗透/image-20220912161524169.png)

####  微步查看域名

`mine.c3pool.com`，确认是矿池域名，此java程序是挖矿病毒

![image-20220912161644145](https://image.201068.xyz/assets/38.后渗透/image-20220912161644145.png)

#### 查看服务器CPU和内存使用率

发现名为`javs.exe`的程序内存使用率极大

![image-20220912161735065](https://image.201068.xyz/assets/38.后渗透/image-20220912161735065.png)

#### javs.exe程序的属性

![image-20220912223108384](https://image.201068.xyz/assets/38.后渗透/image-20220912223108384.png)

####  属性栏中找到程序位置

`C:\Users\Administrator\wkbd\javs.exe`

![image-20220912223142225](https://image.201068.xyz/assets/38.后渗透/image-20220912223142225.png)

#### 上传至微步云沙箱 

进入`C:\Users\Administrator\wkbd\`目录，将该目录打包后上传至微步云沙箱，发现确实是挖矿程序

![image-20220912230823815](https://image.201068.xyz/assets/38.后渗透/image-20220912230823815.png)

![image-20220912231032513](https://image.201068.xyz/assets/38.后渗透/image-20220912231032513.png)

#### 计划任务

 终止 `javs.exe`程序之后，过了几分钟， javs.exe程序又再次出现了，并且消耗CPU 99%，怀疑可能存在**计划任务**

![image-20220912231208314](https://image.201068.xyz/assets/38.后渗透/image-20220912231208314.png)

####  查看任务计划程序

![image-20220912232352788](https://image.201068.xyz/assets/38.后渗透/image-20220912232352788.png)

 在任务计划程序中，发现一个名字为 `system` 的计划任务，计划任务启动的文件是：`C:\Users\Administrator\wkbd\javs.exe`，正是我们之前发现的**挖矿程序**

![c5356097f8ec4597aa1360af6586cf1b.png](https://image.201068.xyz/assets/38.后渗透/c5356097f8ec4597aa1360af6586cf1b.png)

 查看这个挖矿病毒的创建者是`Administrator` ，创建时间是`2022-09-12 16:06:14`

![image-20220912232643840](https://image.201068.xyz/assets/38.后渗透/image-20220912232643840.png)

####  小结

1. 1.挖矿病毒位置：`C:\Users\Administrator\wkbd\`目录 
2. 2.存在挖矿程序的计划任务，任务名字system

删除挖矿程序的计划任务，删除`C:\Users\Administrator\wkbd\` 目录

### 2.2 排查服务器后门

#### 2.2.1 查看可疑进程

使用`Autoruns`工具查看服务器所有的进程，其中背景颜色为粉红色的程序，经排查均不是恶意程序，服务器不存在恶意程序

![image-20220913132758115](https://image.201068.xyz/assets/38.后渗透/image-20220913132758115.png)

![image-20220913132822410](https://image.201068.xyz/assets/38.后渗透/image-20220913132822410.png)

#### 2.2.2 查看启动项

启动项正常

![image-20220913133117981](https://image.201068.xyz/assets/38.后渗透/image-20220913133117981.png)

#### 2.2.3 查看计划任务

计划任务正常，之前的挖矿程序计划任务已删除

![image-20220913133218253](https://image.201068.xyz/assets/38.后渗透/image-20220913133218253.png)

#### 2.2.4 查看服务

未发现可疑服务

![image-20220913151311327](https://image.201068.xyz/assets/38.后渗透/image-20220913151311327.png)

#### 2.2.5 查看镜像劫持

发现了 `shift`粘贴键后门，后门路径是`C:\windows\system32\cmd.exe`，可以在不登陆服务器的情况下执行cmd

![image-20220913171534845](https://image.201068.xyz/assets/38.后渗透/image-20220913171534845.png)
...
![image-20220913173036309](https://image.201068.xyz/assets/38.后渗透/image-20220913173036309.png)

##### 2.管理账户 

通过 控制面板-》用户账户-》管理账户查看当前系统的所有账户未发现异常，只有administrator和guest

![image-20220913173130037](https://image.201068.xyz/assets/38.后渗透/image-20220913173130037.png)

##### 3.注册表 

 通过注册表编辑器，查看发现隐藏账户：`hyf$`

![image-20220913173233347](https://image.201068.xyz/assets/38.后渗透/image-20220913173233347.png)

### 2.3 排查网络连接

使用命令 `netstat -ano` 查看网络连接，未发现异常

![image-20220913174154185](https://image.201068.xyz/assets/38.后渗透/image-20220913174154185.png)
...
3.发现 IP 129.226.111.18，疑似**恶意 IP**
...
![image-20220913174721554](https://image.201068.xyz/assets/38.后渗透/image-20220913174721554.png)
...
将`evtx`工具传到windows server 2008服务器上，因为该服务为64位，

所以进入到 `evtx_0x64` 目录，之后以**管理员**身份运行 evtx.exe 文件

![image-20220913182321706](https://image.201068.xyz/assets/38.后渗透/image-20220913182321706.png)

![image-20220913182358983](https://image.201068.xyz/assets/38.后渗透/image-20220913182358983.png)

###### 生成日志文件

 运行 evtx.exe 文件之后，日志成功提取，

在evtx目录下会生成一些日志文件，如下图所示

![image-20220913182422970](https://image.201068.xyz/assets/38.后渗透/image-20220913182422970.png)

#####  方法二

###### 查看“事件查看器”

![image-20220913182615359](https://image.201068.xyz/assets/38.后渗透/image-20220913182615359.png)

###### 另存为所有事件

 点开 windows日志，之后在右边有个“**将所有事件另存为**”

![image-20220913184300656](https://image.201068.xyz/assets/38.后渗透/image-20220913184300656.png)

 之后就可以**保存所有的安全事件**

![image-20220913184422917](https://image.201068.xyz/assets/38.后渗透/image-20220913184422917.png)

#### 3.2.2 分析安全日志

将提取出来的日志，放到`logon`下的`data`里面(之前的日志需要全部删除)

![image-20220913184815694](https://image.201068.xyz/assets/38.后渗透/image-20220913184815694.png)

![image-20220913184917394](https://image.201068.xyz/assets/38.后渗透/image-20220913184917394.png)

 然后运行`bin`目录里面的`Run.bat`程序即可。

![image-20220913185033650](https://image.201068.xyz/assets/38.后渗透/image-20220913185033650.png)

 如下所示，是运行结束后**统计的各种表格**

![image-20220913185234217](https://image.201068.xyz/assets/38.后渗透/image-20220913185234217.png)

 查看data目录下的4625.csv文件，此文件记录的是所有登录失败的信息，

发现`2022/9/12 15:05:26——2022/9/12 15:09:26`有黑客爆破Administrator账户，但是均没有记录到攻击IP

![fde13a3c326940589afe1a3d4da3d865.png](https://image.201068.xyz/assets/38.后渗透/fde13a3c326940589afe1a3d4da3d865.png)

![image-20220913194634645](https://image.201068.xyz/assets/38.后渗透/image-20220913194634645.png)

 但是在 `2022/9/12 15:06:54` 有审核成功的记录，但是没有IP地址，可能是黑客使用爆破工具成果爆破出Administrator账户密码

![image-20231030024442999](https://image.201068.xyz/assets/38.后渗透/image-20231030024442999.png)

 紧接着 `2022/9/12 15:09:57`，IP：`192.168.1.132` 成功登录该服务器

![image-20220913200354020](https://image.201068.xyz/assets/38.后渗透/image-20220913200354020.png)

 继续排查在 `2022/9/12 15:10:35` 黑客使用administrator账户**创建了隐藏账户** `hyf$`

![image-20220913202243890](https://cdn.jsdelivr.net/gh/jizhihou/image/img/image-20220913202243890.png)

之后在 `2022/9/12 15:10:43` 黑客使用administrator账户将 hyf$​​ 隐藏账户添加到**超级管理员组**，具有超级管理员权限

![image-20220913202917902](https://image.201068.xyz/assets/38.后渗透/image-20220913202917902.png)

![image-20220913203015151](https://image.201068.xyz/assets/38.后渗透/image-20220913203015151.png)

**注意：** `logon`工具使用的前提：电脑需要安装 `LogParser` 工具，不然表格会没有任何数据

LogParser 工具安装参考地址： https://zhuanlan.zhihu.com/p/57092216 https://zhuanlan.zhihu.com/p/57092216?ivk_sa=1024320u
...
- 黑客（IP：192.168.1.132）
- 在 2022/09/12 15:05:26——2022/09/12 15:09:26 对windows server  2008服务器进行暴力破解，
- 并且在 2022/9/12 15:06:54 成功爆破 `administrator` 账户，
- 之后在2022/09/12 15:09:57，IP：192.168.1.132  成功登录该服务器，
- 在 2022/09/12 15:10:35  黑客使用`administrator`账户创建了隐藏账户 `hyf$`，
- 在 2022‎年/09/‎‎12‎  ‏‎15:40:26  植入挖矿程序，
- 在 2022-09-12 16:06:14 创建挖矿程序的计划任务

# Linux应急响应篇

## Linux 网络连接及进程排查

在命令行中输入【netstat -antpu】网络连接命令，可分析可疑端口、可疑IP地址以及PID信息。

```bash
netstat -antpu
```

根据 netstat 命令找到的 PID 的值，可以利用【lsof -p PID】命令，查看进程所打开的文件。

如果确定是恶意进程，可以使用【kill -9 PID】命令结束进程，然后将相关的可以文件删除，

如果 root 用户都无法删除文件，可以使用【lsattr filename】命令查看文件是否被添加了 i 属性，

然后使用【chattr -i filename】移除掉 i 属性，再去删除文件。 
...
对于挖矿进程的排查，可使用【top】命令查看相关资源占用率较高的进程（在 top 命令界面输入 M 可按照内存占用情况排序，默认为按照 CPU 占用排序），之后对其进行重点定位。
...
可以使用【stat /usr/sbin/useradd】【stat /usr/sbin/userdel】命令来查看useradd和userdel命令的时间变化信息

```bash
stat /usr/sbin/useradd
stat /usr/sbin/userdel
```


分析超级权限账户，在命令行中输入【awk -F: '{if ($3==0) print $​1}' /etc/passwd】命令，可查到账户UID为0的账户，如果出现root之外的其他UID为0的账户，需要重点排查。
...
在应急响应中，通过命令【history】可查看历史命令，bash中默认命令记录为1000个。

history 信息默认不显示命令执行的时间，但其实记录了时间，可以通过配置环境变量将时间显示出来

```bash
export HISTTIMEFORMAT='%F %T '
```

命令保存在主文件夹内的“`.bash_history`”中。

## Linux 可疑文件排查

可以对敏感目录的文件进行分析（`/tmp、/usr/bin、/usr/sbin、~/.ssh`）

通过列出攻击日期内变动的文件，可发现相关的恶意软件，可以使用【`find`】命令对某一时间段内增加的文件进行查找。

```bash
-atime -n +n：按文件访问时间来查找文件，-n指n天以内，+n指n天前。
-mtime -n +n：按文件更改时间来查找文件，-n指n天以内，+n指n天前。
-ctime -n +n：按文件改动（指改动了文件属性或元数据）时间来查找文件，-n指n天以内，+n指n天前。
```


也可以通过分析文件的方法对Webshell进行查找，利用Webshell中常出现的一些关键字，对文件进行初筛，缩小排查的范围。

```bash
find /var/www/ -name "*.php" -print0 | xargs -0 egrep 'assert|phpspy|c99sh|milw0rm|eval|gunerpress|base64_decode|sPIDer_bc|shell_exec|passthru|\$\_POST\[eval\(str_rot13\)|\.chr\(|\$\{\"\_P|eval\(\$\_R|file_put_contents\(\.\*\$\_'
```


ls、ps等命令很可能被攻击者恶意替换，所以可以使用 【rpm -Vf /usr/bin/*】【rpm -Vf /usr/sbin/*】对系统命令进行排查，每个文件的输出中可能包含一些标志符号，用于指示文件的不一致情况，例如文件大小、校验和、权限等。
...
启动项是恶意病毒实现持久化驻留的一种常用手段，可以使用以下方法可以查找启动项相关内容。

使用【cat /etc/init.d/rc.local】命令，可查看init.d 文件夹下的rc.local文件内容

使用【ls -alt /etc/init.d】命令，然后查看init.d文件夹下所有文件的详细内容
...
任务计划也是维持权限和远程下载恶意软件的一种手段。在Linux系统中一般有以下两种方法可以查看任务计划。

在命令行中输入【crontab -l】命令，可查看当前的任务计划，也可以指定用户进行查看，如输入命令【crontab -u root -l】，可查看root用户的任务计划。

```bash
crontab -l
```

一般在Linux系统中的任务计划文件是以cron开头的，可以利用 ls 命令配合通配符 * 即【ls /etc/cron*】筛选出etc目录下的所有以 cron 开头的文件，对文件内的详细内容进行排查。

## Linux 服务排查

在 Linux 服务排查时可以使用【systemctl list-unit-files】命令列出系统当中的所有服务信息，在排查时重点排查处于自启动状态的服务。
...
Linux系统中的日志一般存放在目录“/var/log/”下，常见的日志文件功能如下。
...
查找有哪些IP在暴力破解：grep "Failed password" /var/log/secure | awk '{print $11}' | sort | uniq -c | sort -nr

查看登录成功的用户及IP：grep "Accepted" /var/log/secure | awk '{print $9,$11}' |sort | uniq -c | sort -nr

查看登录成功日期、用户名、IP地址：grep "Accepted " /var/log/secure|awk '{print $1,$2,$3,$9,$11}'
...
| Malwoverview 用于威胁搜寻的应急响应工具 | https://github.com/alexandreborges/malwoverview |

# Linux应急响应案例

## 一.事件背景

Linux-暴力破解.替换ps命令并留存后门——事件复现

服务器疑似被入侵，与恶意IP进行通信（因为没有公网IP，所以恶意IP使用192.168.1.132代替）。

**受害服务器：** Centos系统

**IP：** 192.168.1.137.无WEB服务
...
查看服务器的网络连接，发现服务器192.168.1.137一直与恶意IP：192.168.1.132的6666端口连接，并且程序名为：.shell.elf
...
![image-20220906151701104](https://image.201068.xyz/assets/38.后渗透/image-20220906151701104.png)
...
![image-20220906151827112](https://image.201068.xyz/assets/38.后渗透/image-20220906151827112.png)
...
![image-20220906152050707](https://image.201068.xyz/assets/38.后渗透/image-20220906152050707.png)

之后将 .shell.elf 文件放在微步上检测发现，是后门文件

![image-20220906152540996](https://image.201068.xyz/assets/38.后渗透/image-20220906152540996.png)
...
![image-20220906152607699](https://image.201068.xyz/assets/38.后渗透/image-20220906152607699.png)
...
![image-20220906152714482](https://image.201068.xyz/assets/38.后渗透/image-20220906152714482.png)
...
![image-20220906152750086](https://image.201068.xyz/assets/38.后渗透/image-20220906152750086.png)
...
![image-20220906152842817](https://image.201068.xyz/assets/38.后渗透/image-20220906152842817.png)
查看可以远程登录的帐号信息
...
![image-20220906154845339](https://image.201068.xyz/assets/38.后渗透/image-20220906154845339.png)
...
![image-20220906161341490](https://image.201068.xyz/assets/38.后渗透/image-20220906161341490.png)
...
![image-20220906161532690](https://image.201068.xyz/assets/38.后渗透/image-20220906161532690.png)
...
![image-20220906224121436](https://image.201068.xyz/assets/38.后渗透/image-20220906224121436.png)
...
![image-20220906224305094](https://image.201068.xyz/assets/38.后渗透/image-20220906224305094.png)
...
![image-20220906224552311](https://image.201068.xyz/assets/38.后渗透/image-20220906224552311.png)

将 .centos_core.elf 文件放在微步云沙箱检测是后门文件

![image-20220906225911654](https://image.201068.xyz/assets/38.后渗透/image-20220906225911654.png)

而 /.hide_command/ps 文件为正常的ps文件

![image-20220906230136397](https://image.201068.xyz/assets/38.后渗透/image-20220906230136397.png)
...
![image-20220906230736388](https://image.201068.xyz/assets/38.后渗透/image-20220906230736388.png)
...
![image-20220906231853280](https://image.201068.xyz/assets/38.后渗透/image-20220906231853280.png)

查看 定时任务 /var/spool/cron/root 文件的创建时间为：2022-09-06 14:48:08

![image-20220906232244146](https://image.201068.xyz/assets/38.后渗透/image-20220906232244146.png)

查看/.centos_core.elf文件创建时间为：22022-09-06 14:38:00

![image-20220906232523130](https://image.201068.xyz/assets/38.后渗透/image-20220906232523130.png)

查看 ps 命令文件修改时间为：2022-09-06 14:43:31

![image-20220906232644859](https://image.201068.xyz/assets/38.后渗透/image-20220906232644859.png)
...
![image-20220906233122210](https://image.201068.xyz/assets/38.后渗透/image-20220906233122210.png)

![](https://image.201068.xyz/assets/38.后渗透/image-20220906233122210.png)
...
发现 root 账户在9月6 日 14:33:09 登录了该服务器，在爆破的时间范围内（14:31:39——14:33:11） hyf  账户在 9月6 日 16:10:36 登录了该服务器（其中IP地址 192.168.1.132 因为是模拟，没有公网地址，所以假设它是恶意IP）

![image-20220906233536202](https://image.201068.xyz/assets/38.后渗透/image-20220906233536202.png)

### 3.2 事件总结

​	黑客在9月6日14:31:39——14:33:11对服务器进行爆破，且在 14:33:09 成功爆破出root账户密码并在 9月6日 14:33:52 进行登录，登录之后在9月6日14:37:22 植入了 .shell.elf  后门（根据成功爆破出root账户的时间可知 9月5日18:00:06 为 .shell.elf后门的创建时间，并非植入时间）.在 14:38:00 植入了 /.centos_core.elf 后门.在 14:43:31 植入了ps命令后门.在 14:48:08 写了恶意定时任务，恶意IP为192.168.1.132
